...
( function ( $ ) {
	...
	jQuery.fn.fontselect = function ( options ) {
			...
			Fontselect.prototype = {
				keyDown: function ( e ) {
					...
					if ( e.keyCode == 38 ) {
						// Cursor up
						stop( e );
						var $li = jQuery( 'li.blpge_active', this.$results ),
							$pli = $li.prev( 'li' );
						if ( $pli.length > 0 ) {
							$li.removeClass( 'blpge_active' );
							this.$results.scrollTop(
								$pli.addClass( 'blpge_active' )[ 0 ].offsetTop -
					...
					if ( e.keyCode == 40 ) {
						...
						if ( $nli.length > 0 ) {
							$li.removeClass( 'blpge_active' );
							this.$results.scrollTop(
								$nli.addClass( 'blpge_active' )[ 0 ].offsetTop -
									this.searchBoxHeight
							);
						}
						return;
					...
					if ( e.keyCode == 13 ) {
						// Enter
						stop( e );
						jQuery( 'li.blpge_active', this.$results ).trigger(
							'click'
						);
						return;
					}
					...
					if ( $found.length > 0 ) {
						jQuery( 'li.blpge_active', this.$results ).removeClass(
							'blpge_active'
						);
						this.$results.scrollTop(
							$found.addClass( 'blpge_active' )[ 0 ].offsetTop
						);
					}
				},
				...
				bindEvents: function () {
					...
					// Call like so: jQuery("input[name='ffSelect']").trigger('setFont', [fontFamily, fontWeight]);
					this.$original.on(
						'setFont',
						function ( evt, fontFamily, fontWeight ) {
							...
							jQuery(
								'li.blpge_active',
								self.$results
							).removeClass( 'blpge_active' );
							$li.addClass( 'blpge_active' );
					...
					this.$original.on( 'change', function () {
						self.updateSelected();
						self.addFontLink(
							jQuery( 'li.blpge_active', self.$results ).data(
								'value'
							)
						);
					} );
					...
					if ( this.options.searchable ) {
						this.$input.on( 'keypress', function () {
							...
							jQuery( 'li', self.$results ).each( function () {
								if (
									jQuery( this )
										.text()
										.toLowerCase()
				...
				toggleDropdown: function ( hideShow ) {
					if ( hideShow === 'hide' ) {
						...
						this.$element.removeClass(
							'blpge_font-select-blpge_active'
						);
						this.$drop.hide();
				...
				selectFont: function () {
					var font = jQuery( 'li.blpge_active', this.$results ).data(
						'value'
					);
				...
				moveToSelected: function () {
					...
					this.$results.scrollTop(
						$li.addClass( 'blpge_active' )[ 0 ].offsetTop -
							this.searchBoxHeight
					);
				},
				...
				activateFont: function ( e ) {
					...
					jQuery( e.target ).addClass( 'blpge_active' );
				},
				...
				setupHtml: function () {
					...
					this.$search = jQuery( '<div>', { class: 'fs-search' } );
				...
				getVisibleFonts: function () {
					...
					jQuery( 'li', this.$results ).each( function () {
						...
						if ( fb >= top && ft <= bottom ) {
							fs.addFontLink( jQuery( this ).data( 'value' ) );
						}
					} );
				},
				...
				addFontLink: function ( font ) {
					...
					if (this.options.googleFonts.indexOf(font) > -1) {
						jQuery('link:last').after('<link href="' + this.options.googleApi + font + '" rel="stylesheet" type="text/css">');
					}
					else if (this.options.localFonts.indexOf(font) > -1) {
						font = this.toReadable(font);
						jQuery('head').append("<style> @font-face { font-family:'" + font + "'; font-style:normal; font-weight:400; src:local('" + font + "'), url('" + this.options.localFontsUrl + font + ".woff') format('woff'); } </style>");
					}*/
