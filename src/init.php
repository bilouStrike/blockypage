...
{ // phpcs:ignore
	...
	wp_enqueue_script(
		'blpge-front-js',
		plugin_dir_url(__FILE__) . '../dist/customJS/front.js?v=' . time(),
		array('jquery'),
		'',
		true
	);
	...
	wp_enqueue_style(
		'blpge-library-style',
		plugin_dir_url(__FILE__) . '../dist/blpge-library/blpge-style.build.css',
		'',
		plugin_dir_path(__DIR__) . '/dist/blpge-library/blpge-style.build.css?v='.time()
	);
	...
	wp_enqueue_style(
		...
		plugin_dir_path(__DIR__) . '/dist/blocks.style.build.cssv='.time() // Version: File modification time.
	);
}
...
{ // phpcs:ignor
	...
	wp_register_script(
		'blpge-editor-js',
		plugin_dir_url(__FILE__) . '../dist/customJS/editor.js?v=' . time(),
		['wp-data'],
		plugin_dir_path(__DIR__) . '/dist/customJS/editor.js',
		true
	);
	...
	wp_register_script(
		'blpge-library-script',
		plugin_dir_url(__FILE__) . '../dist/blpge-library/blpge-library.build.js',
		['wp-blocks', 'wp-i18n', 'wp-element', 'wp-editor', 'wp-data'],
		plugin_dir_path(__DIR__) . '/dist/blpge-library/blpge-library.build.js?v='.time(),
		false
	);
	...
	wp_register_style(
		...
		'',
		plugin_dir_path(__DIR__) . '/dist/blpge-library/blpge-editor.build.cssv='.time()
	);

	// Font Awesome
	wp_enqueue_style(
		'blpge-font-awesome',
		plugin_dir_url(__FILE__) . '../dist/font-awesome/css/all.min.css'
	);
	...
	wp_enqueue_script(
		...
		plugin_dir_path(__DIR__) . 'dist/blocks.build.jsv='.time(), // Version: File modification time.
	...
	wp_enqueue_style(
		...
		plugins_url('dist/blocks.editor.build.css?v=' . time(), dirname(__FILE__)), // Block editor CSS.
...
function blpge_enqueue_admin_scripts() {
	...
	wp_register_style( 'blpge-admin-css', plugin_dir_url(__DIR__) . 'dist/css/style.css?v='.time().'', '' );
	...
	wp_register_script( 'blpge_admin_js', plugin_dir_url(__DIR__) . 'dist/js/common-script.js?v='.time(), array('jquery'), null, true );
